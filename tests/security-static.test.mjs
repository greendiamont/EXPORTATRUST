...
test("Gmail OAuth uses minimum scopes, encrypted tokens and human review", async () => {
  ...
  assert.match(source, /function mimeHeader/);
  assert.match(source, /Subject: \$\{mimeHeader\(subject\)\}/);
  assert.match(source, /=\?UTF-8\?B\?/);
  assert.match(source, /GMAIL_CLIENT_EMAIL_SENT/);
  assert.match(source, /gmailDeliveryConfiguration/);
  const exportControl = await read("app/api/export-control/route.ts");
  assert.match(exportControl, /sendGmailEmail/);
  assert.match(exportControl, /provider: "Gmail"/);
...
test("stage 01 captures order data and emits official commercial documents", async () => {
  const route = await read("app/api/export-control/route.ts");
  const client = await read("app/client-app.tsx");
  const css = await read("app/globals.css");
  assert.match(route, /action === "order-commercial"/);
  assert.match(route, /ORDER_COMMERCIAL_DETAILS_UPDATED/);
  assert.match(route, /documentTitle/);
  assert.match(route, /sales-order/);
  assert.match(route, /purchase-invoice/);
  assert.match(route, /supplier-po/);
  assert.match(route, /isPurchaseInvoice/);
  assert.match(route, /supplierOrder/);
  assert.match(route, /PEDIDO DE COMPRA/);
  assert.match(route, /FORNECEDOR BRASIL/);
  assert.match(route, /ITENS DO PEDIDO/);
  assert.match(route, /Assinatura do fornecedor/);
  assert.match(route, /supplier\?\.bankDetails/);
  assert.match(route, /<strong>BANK DETAILS<\/strong>/);
  assert.match(route, /footerBlock/);
  assert.match(route, /DEFAULT_ORDER_NOTES/);
  assert.match(route, /orderItems/);
  assert.match(route, /commercialValue: orderItems\.length \? totalValue : safeNumber\(body\.commercialValue\)/);
  assert.match(route, /paymentTerms/);
  assert.match(route, /@page\{size:A4/);
  assert.match(route, /table-layout:fixed/);
  assert.match(route, /page-break-inside:avoid/);
  assert.match(route, /Importer \/ Buyer signature/);
  assert.match(route, /Exporter \/ Supplier signature/);
  assert.match(client, /ETAPA 01 · PEDIDO/);
  assert.match(client, /Fornecedor Brazil/);
  assert.doesNotMatch(client, /Tax ID exportador/);
  assert.match(client, /orderItemsTotal/);
  assert.match(client, /supplierOrderDraft/);
  assert.match(client, /supplierOrderItems/);
  assert.match(client, /PEDIDO DE COMPRA FORNECEDOR/);
  assert.match(client, /Condição de pagamento fornecedor/);
  assert.match(client, /Preço fornecedor/);
  assert.match(client, /commercialValue: orderItemsTotal/);
  assert.match(client, /Somatório automático dos totais dos itens/);
...
test("supply chain tab has per-order tasks with due dates and owners", async () => {
  const schema = await read("db/schema.ts");
  const route = await read("app/api/export-control/route.ts");
  const client = await read("app/client-app.tsx");
  const css = await read("app/globals.css");
  const migration = await read("drizzle/0026_careful_sharon_ventura.sql");
  assert.match(schema, /operationTasks/);
  assert.match(schema, /operation_tasks/);
  assert.match(schema, /operation_tasks_operation_sequence_idx/);
  assert.match(schema, /responsibleEmail/);
  assert.match(schema, /scheduled/);
  assert.match(migration, /CREATE TABLE `operation_tasks`/);
  assert.match(route, /DEFAULT_OPERATION_TASKS/);
  assert.match(route, /operation-task-create/);
  assert.match(route, /operation-task-update/);
  assert.match(route, /operation-task-toggle-scheduled/);
  assert.match(route, /operation-task-complete/);
  assert.match(route, /operation-task-delay-note/);
  assert.match(route, /TASK_DELAY/);
  assert.match(route, /taskAlerts/);
  assert.match(client, /tab === "overview"[\s\S]*taskBoardOnly/);
  assert.match(client, /if \(taskBoardOnly\) return taskBoard/);
  assert.match(client, /SUPPLY CHAIN TASKS/);
  assert.match(client, /Tarefas e subtarefas do pedido/);
  assert.doesNotMatch(client, /Controle diário no estilo Asana/);
  assert.match(client, /\+ tarefa/);
  assert.match(client, /Marcar agendada/);
  assert.match(client, /Enviar nota atraso/);
  assert.match(client, /taskDrafts/);
  assert.match(client, /task-sequence/);
  assert.doesNotMatch(client, /Número da tarefa/);
  assert.match(css, /operation-overview > \.operation-task-board/);
  assert.match(css, /operation-task-board/);
  assert.match(css, /operation-task-row\.overdue/);
});
