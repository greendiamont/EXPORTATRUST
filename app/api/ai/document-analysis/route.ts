...
export async function GET() {
  try {
    ...
    return Response.json({
      ...
      model: env.OPENAI_DOCUMENT_MODEL || "gpt-5.6-terra",
      supported: ["pdf", "doc", "docx", "xls", "xlsx", "csv", "txt", "json", "jpg", "jpeg", "png", "webp", "gif"],
...
export async function POST(request: Request) {
  try {
    ...
    const body = await request.json() as { documentId?: number; operationContext?: string };
    ...
    const [document] = await db
      .select()
      .from(operationDocuments)
      .where(and(
        eq(operationDocuments.id, documentId),
    ...
    const [operation] = await db
      ...
      .from(operations)
      .where(and(
        eq(operations.id, document.operationId),
        eq(operations.organizationId, context.organizationId),
      ))
      .limit(1);
    ...
    if (!env.OPENAI_API_KEY) {
      return Response.json({ error: "Leitura por IA ainda não foi configurada no ambiente de execução." }, { status: 503 });
    }
    if (!env.BUCKET) {
    ...
    const registeredContext = [
      `Referência: ${operation.reference}`,
      `Fornecedor/exportador: ${operation.supplierName || "não informado"}`,
      `Importador: ${operation.euImporter || "não informado"}`,
      `País de destino: ${operation.destinationCountry || "não informado"}`,
      body.operationContext?.trim() || "",
    ].filter(Boolean).join("; ");
    ...
    const analysis = await analyzeDocumentWithOpenAI({
      ...
      operationContext: registeredContext,
    });

    await audit(context, "DOCUMENT_ANALYZED_BY_AI", "operation_document", String(document.id), {
    ...
    const status = /OpenAI|formato|documento|análise/i.test(message) ? 422 : 500;
